...
contract AssetLogging {
    ...
    function registerAdmin(
        string memory _name,
        address _adminPubkey,
        uint8 _type
    ) public onlyOwner {
        if (
            admins[_adminPubkey].isRegistered ||
            assets[_adminPubkey].isRegistered
        ) {
            emit RejectRegistration(
                "This address is already in use or sender is not authorized"
            );
            return;
        }
        ...
        emit NewAdmin(
            admins[_adminPubkey].adminName,
            admins[_adminPubkey].adminPubkey,
            admins[_adminPubkey].adminType,
            admins[_adminPubkey].isRegistered,
            admins[_adminPubkey].registrationTime
        );
    }
    ...
    function registerAsset(
        ...
        if (!isOwner(_ownerPubkey)) {
            emit RejectRegistration(
                "The admin is not registered or not an owner"
            );
            return;
        }
        if (!isDso(_dsoPubkey)) {
            emit RejectRegistration(
                "The admin is not registered or not an DSO"
            );
            return;
        }
        if (
            assets[_assetPubkey].isRegistered ||
            admins[_assetPubkey].isRegistered
        ) {
            emit RejectRegistration("This address is already in use.");
            return;
        }

        assets[_assetPubkey] = Asset(
            _name,
            _type,
            _ownerPubkey,
            _dsoPubkey,
        ...
        emit NewAsset(
            assets[_assetPubkey].assetOwner,
            assets[_assetPubkey].assetType,
            assets[_assetPubkey].ownerPubkey,
            assets[_assetPubkey].dsoPubkey,
            assets[_assetPubkey].assetPubkey,
            assets[_assetPubkey].voltageLevel,
            assets[_assetPubkey].peakPower,
            assets[_assetPubkey].isRegistered,
            assets[_assetPubkey].registrationTime
        );
    }
    ...
    function transferAsset(address _to, address _assetPubkey) public {
        if (!isAssetRegistered(_assetPubkey)) {
            emit RejectTransfer(
                ...
                _to,
                _assetPubkey,
                "No asset with this pubkey is registered"
            );
            return;
        }
        if (admins[msg.sender].adminType != uint8(1)) {
            emit RejectTransfer(
                ...
                _to,
                _assetPubkey,
                "Sender is not registered as an owner"
            );
            return;
        }
        if (admins[_to].adminType != uint8(1)) {
            emit RejectTransfer(
                ...
                _to,
                _assetPubkey,
                "Receiver is not registered as an owner"
            );
            return;
        }
        if (!ownership[msg.sender][_assetPubkey]) {
            emit RejectTransfer(
                ...
                _to,
                _assetPubkey,
                "Sender is not owner of this asset"
            );
            return;
        }
    ...
    function changeDSO(address _to, address _assetPubkey) public {
        ...
        if (!isDso(msg.sender)) {
            emit RejectTransfer(
                ...
                _to,
                _assetPubkey,
                "Sender is not registered as DSO"
            );
            return;
        }
        if (!isDso(_to)) {
            emit RejectTransfer(
                ...
                _to,
                _assetPubkey,
                "Receiver is not registered as DSO"
            );
            return;
        }
        if (!ownership[msg.sender][_assetPubkey]) {
            emit RejectTransfer(
                ...
                _to,
                _assetPubkey,
                "Sender is not DSO of this asset"
            );
            return;
        }
    ...
    function isOwnerOf(address _ownerPubkey, address _assetPubkey)
        public
        view
        returns (bool)
    {
        if (
            !ownership[_ownerPubkey][_assetPubkey] ||
            admins[_ownerPubkey].adminType != uint8(1)
        ) {
            return false;
        }
    ...
    function getAsset(address _pubkey)
        public
        view
        returns (
            string memory,
    ...
    {
        return (
            assets[_pubkey].assetType,
            assets[_pubkey].ownerPubkey,
            assets[_pubkey].dsoPubkey,
            assets[_pubkey].isRegistered
        );
    }
    ...
    function getAdmin(address _pubkey)
        public
        view
        returns (
            address,
    ...
    {
        return (
            admins[_pubkey].adminPubkey,
            admins[_pubkey].adminName,
            admins[_pubkey].adminType,
            admins[_pubkey].isRegistered
        );
    }
    ...
    function isAssetRegistered(address _assetPubkey)
        internal
        view
        returns (bool)
    {
    ...
    function setDsoValue(address _assetPubkey, uint256 _value) public {
        if (!isDso(msg.sender) || !ownership[msg.sender][_assetPubkey]) {
            emit RejectSetValue(
                msg.sender,
                "Sender is either not registered as DSO or DSO of the asset or asset is not registered"
            );
            return;
        }
    ...
    function getDsoLatestValue(address _assetPubkey)
        public
        view
        returns (uint256)
    {
    ...
    function setFlexibility(
        string memory _startTime,
        string memory _endTime,
        uint256 _reductionLevel,
        uint256 _price
    ) public {
        require(
            assets[msg.sender].isRegistered,
            "Registration of asset is required"
        );
    ...
    function getFlexibility(address _asset)
        public
        view
        returns (
            uint256,
    ...
    {
        return (
            flexibilities[_asset].timestamp,
            flexibilities[_asset].startTime,
            flexibilities[_asset].endTime,
            flexibilities[_asset].reductionLevel,
            flexibilities[_asset].price
        );
    }
}
